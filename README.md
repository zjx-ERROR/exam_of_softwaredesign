...
- 数据传输控制方式
    ...
    2. 中断控制方式
        1. 在I/O中断方式下，数据的输入按如下步骤操作
            1. 需要数据的进程通过CPU发出启动外设输入数据的指令，改指令同时置状态寄存器的中断允许位
            2. 在进程发出指令启动设备之后，改进程放弃处理机，等待输入完成。这时，进程调度程序可以调度其他就绪进程占据处理机
            ...
            4. 在以后的某个时刻，进程调度程序选中提出请求输入的进程，改进程从指定的内存单元中取出数据做进一步处理
        2. 中断控制方式的优缺点
